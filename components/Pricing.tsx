...
export const Pricing: React.FC = () => {
  ...
  const plans = useMemo(
    () =>
      TIERS.map((tier) => ({
        ...tier,
        price: tier.monthly * multipliers[cycle],
      })),
    [cycle],
  );
          ...
          <div className="join bg-white p-1 rounded-full border border-slate-300">
            {(['mensal', 'semestral', 'anual'] as Cycle[]).map((value) => (
              <button
                ...
                className={`join-item btn btn-sm rounded-full px-6 ${cycle === value ? 'bg-primary hover:bg-primary-dark text-white border-none' : 'bg-white border-none text-slate-700'}`}
              >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
          {plans.map((plan) => (
            <article key={plan.name} className={`rounded-2xl border p-6 bg-white shadow-sm ${plan.highlight ? 'border-primary shadow-blue-100' : 'border-slate-200'}`}>
              ...
              <a
                href="https://staging.quobo.io/"
                className={`btn mt-6 w-full ${plan.highlight ? 'bg-primary hover:bg-primary-dark text-white border-none' : 'bg-slate-50 border border-slate-300 text-slate-700 hover:bg-slate-100'}`}
              >
                {plan.cta}
              </a>
            </article>
